...
void VideoUla::reset_shift_clk() {
  switch (shift_clk_) {
    case 16:shift_countdown_ = 1;
      break;
    case 8:shift_countdown_ = 2;
      break;
    case 4:shift_countdown_ = 4;
      break;
    case 2:shift_countdown_ = 8;
      break;
  }
}
...
void VideoUla::maybe_drive_crtc(const std::shared_ptr<Bus> &main_bus, const std::shared_ptr<Bus> &dram_bus) {
  if (crtc_) {
    ...
    if (clock_->went_low(CLK_1_MHZ) ||
      ...
      // TODO: Remove me, I'm debug code here to check that in Mode 4, addresses
      // are correctly generated by CRTC
      static bool logging_on = false;
      static uint32_t last_socl = 0;
      auto addr = dram_bus->get_address();
      if( addr == 0x5800 ) {
        logging_on = true;
        last_socl = 0x5800;
      }
      if( logging_on) {
        auto SOCL = ((addr - 0x5800) % 320 == 0);
        if( SOCL ) {last_socl = addr;}
        auto SOL = ((addr - last_socl) < 8);
        spdlog::error("CRTC generated 0x{:04x} {}", addr, SOCL?"SOCL":(SOL?"SOL":""));
      }
      if( addr == 0x7fff ) {
        logging_on = false;
      }
