...
void check_address_range(uint16_t base_addr, uint16_t sz) {
  ...
  auto msg = fmt::format("ROM: Can't allocate {} bytes at_bus {:04x} as it exceeds 64K addressable space.", sz,
                         base_addr);
  spdlog::critical(msg);
  throw std::runtime_error(msg);
}
...
Rom::Rom(const std::string &file_name, uint16_t bus_addr) //
  ...
  memory_ = std::make_shared<std::vector<uint8_t>>((std::istreambuf_iterator<char>(file)),
